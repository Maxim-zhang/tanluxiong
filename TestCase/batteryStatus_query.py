...
class batWebStatusQuery(object):
    ...
    def status_query(self, start=2, end=1000, type = '3', by='devid'):
        # 登录GPS管理系统
        ...
        for n in range(start, end+1):
            ...
            if value is None:
                ...
                if validBatRow[1] > 0:
                    ...
                    for (a,b) in  [(batInfo[0],batStatus[0]),(batInfo[2],batStatus[1]),(batInfo[1],batStatus[2])]:
                        if a == b or a =='':
                            continue
                        else:
                ...
                #查询数据为空时处理：DEVID、IMEI、IMSI保持不变,type、其它内容同样置空,备注暂无数据
                    ...
                    batStatus = [batInfo[0],batInfo[1],batInfo[2],'','','','','','NG','暂无数据','']
...
if __name__ == '__main__':
    ...
    cfg7 = [file, 'WX', 2, 50, by]
    cfg_all = [cfg7,cfg1,cfg2,cfg4,cfg5,cfg6,cfg3]

    for cfg in cfg_all:
        rsp = batWebStatusQuery(filename=cfg[0], sheetname=cfg[1])
        # 设置查询开始行数、结束行数、查询方式：devid、imei、imsi
        rsp.status_query(start=cfg[2], end=cfg[3], by=cfg[4])
        rsp.web_close()